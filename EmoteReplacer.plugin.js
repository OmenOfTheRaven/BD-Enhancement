...
const PLUGIN_CHANGELOG = [
  {
    title: '2.2.6',
    type: 'fixed',
    items: ['Fixed custom emote autocomplete window', 'Fixed upload permissions check'],
  },
];
...
class CompletionsService extends BaseService {
  ...
  doRenderCompletions() {
    ...
    this.htmlService.addClasses(
      autocompleteDiv,
      discordClasses.Util.autocomplete,
    ...
    this.htmlService.addClasses(autocompleteInnerDiv, discordClasses.Util.autocompleteInner);
...
class ModulesService extends BaseService {
  ...
  start() {
    ...
    this.isExternalEmojiAllowedForIntentionModule = BdApi.Webpack.getModule(
      BdApi.Webpack.Filters.byKeys('isExternalEmojiAllowedForIntention')
    );
...
class SendMessageService extends BaseService {
  ...
  async onSendMessage(args, original) {
    ...
    for (const stickerId of stickerIds) {
      const url = this.getStickerUrl(stickerId, channelId);
      if (!url) continue;
    ...
    if (stickerUrls.length > 0) {
      // Send as plain URLs instead of uploading a converted attachment,
      // so Discord renders them as link embeds.
      attachmentData.stickerIds = undefined;

      const existingContent = message.content ? `${message.content}\n` : '';
      message.content = existingContent + stickerUrls.join('\n');
  ...
  getStickerUrl(stickerId, channelId) {
    ...
    switch (sticker.format_type) {
      case formatType.APNG:
        // Discord's CDN can't transcode APNG to gif (returns 415 Unsupported
        // Media Type) — that transcoding only works for Lottie stickers.
        // Keep the native .png; it does animate when the image is loaded
        // directly, though Discord's embed preview may render it static.
        extension = 'png';
        break;
      case formatType.GIF:
        extension = 'gif';
        break;
      case formatType.LOTTIE:
        // Lottie stickers are vector animations with no static png/webp
        // rendition stored on Discord's CDN (fetching one gives a
        // NoSuchKey error). Discord can transcode them to a gif on the
        // fly instead, so request that format for these.
        extension = 'gif';
        break;
      default:
        extension = 'png';
    ...
    // media.discordapp.net (Discord's own image proxy) supports width/height
    // query params that actually control the rendered embed size — unlike
    // cdn.discordapp.com's "size" param, which only changes the resolution
    // of the fetched asset but not how big Discord displays it in the embed.
    // Discord's CDN/proxy only accepts power-of-two sizes (16-4096), so
    // round to the nearest valid value.
    const size = this.nearestPowerOfTwo(this.settingsService.settings.emoteSize);
    return `https://media.discordapp.net/stickers/${stickerId}.${extension}?width=${size}&height=${size}`;
  }

  nearestPowerOfTwo(value) {
    const clamped = Math.min(Math.max(value, 16), 4096);
    const power = Math.round(Math.log2(clamped));
...
class PatchesService extends BaseService {
  ...
  start(sendMessageService, attachService, completionsService, emoteService, modulesService) {
    ...
    this.externalEmojiCategoriesPatch();
  ...
  lockedEmojisPatch() {
    ...
    this.patcher.after(emojiStore, 'getEmojiUnavailableReasons', (_, args, result) => {
      // canUseEmojisEverywhere/canUseAnimatedEmojis (the Nitro "use emoji
      // anywhere" perk) can't be patched directly — Discord defines them
      // as non-configurable exports, so reassigning throws. Instead of
      // patching those, clean up their effect here: without Nitro, every
      // emoji from a guild other than the active one gets marked
      // PREMIUM_LOCKED, which fills emojisDisabled and can make the
      // picker treat the whole guild section as unusable/hidden. Clear
      // that out so all emoji show as fully available.
      result.emojiNitroLocked = false;
      result.emojisDisabled = new Set();
      result.emojisPremiumLockedCount = 0;
      return result;
    });
  ...
  externalEmojiCategoriesPatch() {
    const module = this.modulesService.isExternalEmojiAllowedForIntentionModule;
    if (!module?.isExternalEmojiAllowedForIntention) return;

    // The emoji-picker's per-guild category builder (useEmojiCategories)
    // calls this to decide (a) whether a guild's emoji should be treated
    // as "internal" so they don't get filtered out of that guild's own
    // section, and (b) whether non-current-guild sections get stripped
    // from the CUSTOM category entirely. Forcing this true makes every
    // guild you're in show up in the emoji picker's category rail/body,
    // even in channels without "Use External Emojis" permission.
    this.patcher.instead(
      module,
      'isExternalEmojiAllowedForIntention',
      () => true
    );
  }
